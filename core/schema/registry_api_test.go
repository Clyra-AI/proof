...
func TestScopedRegistryConvenienceAPIsAndManifestFile(t *testing.T) {
	...
	rawSchema := []byte(`{"type":"object","required":["record_type"],"properties":{"record_type":{"const":"vendor.file"}}}`)
	...
	require.NoError(t, r.RegisterCustomTypeSchema("vendor.file2", "urn:test:file2", "1", "schemas/vendor-file.json"))
	require.Len(t, r.Definitions(), 2)
	require.Len(t, r.ListRecordTypes(), len(builtins)+2)
	require.Len(t, r.Manifest().RecordTypes, 2)
